    ...
    impl PortalController {
        ...
        pub fn next_rotation(&mut self, mouse_x:i32, mouse_y: i32) -> f32 {
            if self.should_rotate {
                if (mouse_x as f32) > self.physics.x()+34.0 {
                    self.wand_rotation = ((mouse_y as f32-(self.physics.y()+50.0))/(mouse_x as f32-(self.physics.x()+34.0))).atan()*57.29;
                } else {
                    self.wand_rotation = 180.0 + ((mouse_y as f32-(self.physics.y()+50.0))/(mouse_x as f32-(self.physics.x()+34.0))).atan()*57.29;
                }
            }
            self.wand_rotation
        }
