...
fn main() {
    ...
    for stream in listener.incoming() {
        ...
        /* NOT GREAT CAN EAT UP RESOURCES
        //This code spawns a new thread for every connection
        thread::spawn(|| {
            handle_connection(stream);
        });
        */
...
fn handle_connection(mut stream: TcpStream) {
    ...
    //If we request /sleep from our root
    ...
    let(status_line, filename) = if buffer.starts_with(get) {
        ...
        //Simulates that the response will take at least 5 seconds
